...
Response::ResponseCode Server_Player::cmdCreateRandom(const Command_CreateRandom &cmd, ResponseContainer &rc, GameEventStorage &ges)
{
	if (spectator)
		return Response::RespFunctionNotAllowed;
	...
	Event_CreateRandom event;
	event.set_cmc(cmd.cmc());
	event.set_comparator(cmd.comparator());
	event.set_type(cmd.type());
...
Response::ResponseCode Server_Player::processGameCommand(const GameCommand &command, ResponseContainer &rc, GameEventStorage &ges)
{
    switch ((GameCommand::GameCommandType) getPbExtension(command)) {
		...
		case GameCommand::CREATE_RANDOM: return cmdCreateRandom(command.GetExtension(Command_CreateRandom::ext), rc, ges); break;
